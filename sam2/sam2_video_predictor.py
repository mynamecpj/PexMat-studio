...
class SAM2VideoPredictor(SAM2Base):
    ...
    def init_state(
            ...
            progress_callback=None,  # 新增：接收进度回调
    ):
        ...
        images, video_height, video_width = load_video_frames(
            ...
            progress_callback=progress_callback,  # 向下传递给加载函数
        )
        inference_state = {}
    ...
    def propagate_in_video(
        ...
        """Propagate the input points across frames to track in the entire video (With Sliding Window Memory Release)."""
        import gc  # 确保导入
        import time
        ...
        # ★ 滑动窗口大小：保留最近的帧数。必须大于 SAM2 注意力机制需要的帧数（self.num_maskmem 通常是 7）
        # 我们设置为 15，确保接力连贯且显存占用恒定
        keep_frames = max(15, getattr(self, 'num_maskmem', 7) + 5)
        frames_processed = 0

        for frame_idx in tqdm(processing_order, desc="propagate in video", disable=True):
            time.sleep(0.005)  # 强行挂起 5 毫秒，释放 GIL 给 UI 动画，防假死
            ...
            # --- ★★★ 核心修复：执行内存截断与接力清理 ★★★ ---
            frames_processed += 1
            # 每 10 帧触发一次深度清理，避免每帧清理造成的字典操作开销
            if frames_processed % 10 == 0:
                self._sliding_window_memory_clean(inference_state, frame_idx, keep_frames, reverse, batch_size)

            yield frame_idx, obj_ids, video_res_masks

    def _sliding_window_memory_clean(self, inference_state, current_frame_idx, keep_frames, reverse, batch_size):
        """
        ★ 核心接力引擎：清除远离当前帧的历史特征和结果字典，让显存/内存永远保持在健康水位。
        保留条件帧 (cond_frame_outputs)，只清理非条件帧 (non_cond_frame_outputs) 和 图像主干特征 (cached_features)。
        """
        import gc
        import torch
        ...
        # 1. 清理每个对象的非条件帧结果字典
        for obj_idx in range(batch_size):
            non_cond = inference_state["output_dict_per_obj"][obj_idx]["non_cond_frame_outputs"]

            # 找到超出保留窗口的历史帧 Key
            if not reverse:
                keys_to_delete = [k for k in non_cond.keys() if k < current_frame_idx - keep_frames]
            else:
                keys_to_delete = [k for k in non_cond.keys() if k > current_frame_idx + keep_frames]

            for k in keys_to_delete:
                old_out = non_cond.pop(k, None)
                # 物理销毁底层张量
                if old_out is not None:
                    if "maskmem_features" in old_out and old_out["maskmem_features"] is not None:
                        del old_out["maskmem_features"]
                    if "pred_masks" in old_out and old_out["pred_masks"] is not None:
                        del old_out["pred_masks"]
                    del old_out

        # 2. 清理极其占用显存的视觉骨干网络特征缓存
        cached_features = inference_state.get("cached_features", {})
        if not reverse:
            feat_keys_to_delete = [k for k in cached_features.keys() if k < current_frame_idx - keep_frames]
        else:
            feat_keys_to_delete = [k for k in cached_features.keys() if k > current_frame_idx + keep_frames]

        for k in feat_keys_to_delete:
            old_feat = cached_features.pop(k, None)
            if old_feat is not None:
                del old_feat

        # 3. 清理已追踪帧记录 (可选项，占用极小，但为了保持字典干净)
        for obj_idx in range(batch_size):
            tracked = inference_state["frames_tracked_per_obj"][obj_idx]
            if not reverse:
                track_keys_to_del = [k for k in tracked.keys() if k < current_frame_idx - keep_frames]
            else:
                track_keys_to_del = [k for k in tracked.keys() if k > current_frame_idx + keep_frames]
            for k in track_keys_to_del:
                tracked.pop(k, None)

        # 4. 强制调用垃圾回收并清空 PyTorch CUDA 碎片缓存
        gc.collect()
        device = inference_state.get("device")
        if device is not None and device.type == "cuda":
            torch.cuda.empty_cache()
            # ipc_collect 有助于释放跨进程通讯残留
